#!/bin/bash
# LoRA v8 (no lm_head, lr=5e-4): Qwen3-4B-Base + OpenThoughts3 100K
# Test: high lr without lm_head instability
# lr=5e-4, α=256 (effective LoRA lr = 1e-3), no modules_to_save
# p5-4 (master) + p5-3 (worker), 16×H100, DDP, ~18s/step
# Usage:
#   Master (p5-4): bash run-sft-lora-v8-multinode.sh master
#   Worker (p5-3): bash run-sft-lora-v8-multinode.sh worker
set -ex
...
torchrun \
    ...
    --output_dir /root/.cache/huggingface/sft-qwen3-4b-lora-v8-nolmhead \
    --tuner_type lora \
