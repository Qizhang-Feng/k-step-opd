#!/bin/bash
# LoRA v5 multi-node: Qwen3-4B-Base + OpenThoughts3 100K
# lr=3e-5, α=128, + modules_to_save lm_head
# p5-4 (master) + p5-3 (worker), 16×H100, DDP, ~17s/step
# Result: AIME 0% (lr too low, model learned thinking but degenerated on hard problems)
# Usage:
#   Master (p5-4): bash run-sft-lora-v5-multinode.sh master
#   Worker (p5-3): bash run-sft-lora-v5-multinode.sh worker
set -ex

ROLE=${1:?Usage: $0 <master|worker>}
MASTER_ADDR=172.31.0.227
MASTER_PORT=29501
NUM_NODES=2
NUM_GPUS=8
...
torchrun \
    ...
    --output_dir /root/.cache/huggingface/sft-qwen3-4b-lora-v5-multinode \
    --tuner_type lora \
