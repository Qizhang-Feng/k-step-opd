#!/bin/bash
# Full FT: Qwen3-8B-Base + teacher_sft_179k_merged (179K, unfiltered)
# Following Lightning-OPD config: full FT, ZeRO-1, liger, packing, lr=8e-5
# Single node 8×H100, global batch = 8*2*2 = 32 (vs Lightning-OPD's 128 on 32 GPU)
# To match Lightning-OPD's effective training: max_steps=3000
# Usage: docker exec k-step-opd-sft bash /workspace/k-step-opd/scripts/run-sft-full-8b-179k.sh
set -ex
...
torchrun --nproc_per_node 8 \
    ...
    --output_dir ${OUTPUT_PATH:-/root/.cache/huggingface/sft-qwen3-8b-full-179k} \
    --tuner_type full \
